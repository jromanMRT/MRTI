...
| Nuevo módulo. Alta de MRTI Legal | Parcial — código completo, despliegue pendiente por bloqueo de base de datos | 2026-08-31 | Legal `f4f1e5b` (repositorio nuevo `MRTI-Legal/`); Core `ad4a4f3`. Módulo de expedientes legales y documentación jurídica confidencial: expediente con clave `LEG-000001`, tipo, estado, área y fechas; documentos con versionado inmutable (SHA-256, MIME real por firma binaria, nunca sobrescribe); confidencialidad interno/confidencial/restringido/privilegiado con autorización explícita adicional para los dos últimos niveles (el creador que clasifica un documento recibe esa autorización automáticamente, para no perder acceso a lo que acaba de subir — hallazgo real de las pruebas de contrato, no supuesto de diseño); permisos locales por expediente (`admin_legal`/`responsable`/`editor`/`consulta`/`auditor`) validados en cada endpoint del backend, nunca sólo ocultando botones; retención configurable y bloqueo legal que impide archivar/eliminar mientras esté activo; auditoría local completa con intentos rechazados. Sin llaves foráneas hacia otras bases; usuarios referenciados por UUID de Core. Core: migración idempotente `013_register_mrti_legal.sql` (catálogo `mrti-legal`), `mrti-legal` sumado a `auditSources.js` (mismo contrato que RH/Activos/Obs/Tickets) y a la campanilla global (`notificationSources.js`/`notificationRoutes.js`, vencimientos de contrato y revisiones pendientes vía `/api/legal-self/notifications`, en paralelo con Tickets), ícono de campanilla distinguido por módulo en `main.js`, bloques Nginx `/legal-api/`+`/mrti-legal/` preparados en `deploy/nginx.conf.example`. Frontend con el shell compartido de Core: `useTheme.js` y `PortalNotifications.jsx` copiados sin cambios, tokens visuales, sidebar con logo/tema/campanilla/cerrar sesión igual que RH/Activos/Tickets/Obs. Evidencia: 10/10 pruebas unitarias (saneamiento de auditoría, validación de archivos); migraciones `001`-`004` aplicadas dos veces contra una base de pruebas desechable (`mrtops`, con permisos temporales) sin cambios en la segunda corrida (9 tablas antes y después); 31/31 verificaciones de contrato end-to-end contra el backend real con esa base y un doble de Core (sin token 401, sin módulo 403, administrador acceso completo, permiso por expediente, documento restringido sin autorización 403, versionado sin sobrescritura con hashes distintos, descarga con contenido idéntico byte a byte, archivo `.exe` rechazado, bloqueo legal impide archivar 409, aislamiento entre expedientes, auditoría de descargas/negaciones/bloqueo legal, auditoría consolidada sólo para administrador); build de Core y `git diff --check` correctos; fixtures, base de pruebas y binario del backend detenidos y eliminados al terminar (0 residuos verificados). **Pendiente, documentado en `MRTI-Legal/README.md`:** crear la base `mrti_legal` y otorgar `GRANT ALL` a `mrtops` (requiere acceso root de MySQL, mismo bloqueo histórico que la creación original de `mrti_core` en la Fase 3 — ver §4); cifrado en reposo (el servidor bloquea cargas reales con `LEGAL_ALLOW_REAL_DOCUMENTS=false` hasta resolverlo); activar los bloques de Nginx y registrar `mrti-legal-api` en pm2 (deliberadamente no se hizo: haría visible un enlace roto en el catálogo para administradores antes de tener base de datos). No se cargó ningún documento legal real ni se guardó ningún secreto. Rollback: ver `MRTI-Legal/README.md` §Rollback — ambos repos quedan con cambios aditivos, nada destructivo. |
