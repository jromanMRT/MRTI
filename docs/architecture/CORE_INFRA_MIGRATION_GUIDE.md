...
| Transversal. Core instalable en móviles | En progreso — publicada, HTTPS aún recomendado | 2026-09-01 | Core incorpora manifiesto PWA, `service worker` que excluye APIs del caché, pantalla sin conexión, modo independiente, captura frontal para la foto de perfil y avisos derivados de la campanilla mientras la aplicación permanece activa. El modo HTTP ofrece sonido, título y tarjeta flotante; para PCs Windows corporativas publica preparación y reversión que agregan únicamente `http://192.168.1.203/` a `OverrideSecurityRestrictionsOnInsecureOrigin` y `NotificationsAllowedForUrls` de Chrome/Edge, sin desactivar protecciones globales. Build y sintaxis correctos; shell, manifiesto, service worker, pantalla offline y las dos herramientas responden 200. HTTPS continúa siendo la solución definitiva y necesaria para teléfonos no administrados. No hay migración, backend ni datos modificados. Procedimiento y rollback en `docs/mobile/PWA_DEPLOYMENT.md`. |

| Integración de plataforma. Fase 1: Activos ↔ Tickets | Completa | 2026-09-02 | Activos `5135dfd`; MRTI-Infra `d5eae2b`; Tickets `82ae1b3`; Core `24b9a2d`. Primera fase de la integración pedida por el usuario entre MRTI-Activos y el resto de la plataforma (Tickets, RH, Monitor/Agent, Legal, Core), identificadores canónicos, sin FK entre bases. `tickets.asset_uid` (VARCHAR(64), nullable, indexado) referencia el `asset_uid` de Activos, mismo criterio que `related_device_id`/`requester_id`/`assigned_to`. Se introdujo el primer mecanismo de credencial de servicio de la plataforma (`INTERNAL_SERVICE_KEY`, header `X-Service-Key`, compartida entre Activos/Infra/Tickets) para llamadas automáticas sin usuario navegando, sin tocar el patrón existente de `authRequired` por sesión. Nuevos endpoints de autoservicio de solo lectura, no sensibles (nunca credenciales): `GET /api/activos-self/uid/:assetUid` en Activos y `GET /api/self/devices/:id` en MRTI-Obs — este último corrige de paso una deuda técnica real: `mrtiInfraClient.ts`'s `getDeviceInfo()` llamaba a un endpoint (`/api/devices/:id`) que nunca existió en Infra y fallaba en silencio; ahora resuelve `device_id -> asset_uid` de verdad (`devices.asset_id` ya es el `asset_uid` de Activos desde la Fase 6). UI: pestaña "Tickets" (abiertos/cerrados, lectura en vivo, sin copiar datos) y botón "Crear ticket" en el popup del activo; el botón abre Core con `?openTicket=1&asset_uid=...`, que preselecciona el activo en el formulario de autoservicio existente (`/api/tickets-self`) sin necesidad de abrir el módulo operativo de Tickets. `TicketDetail` muestra una tarjeta "Activo relacionado" leída en vivo de Activos por el mismo mecanismo de origen compartido por Nginx que ya usa `obsFetch` (sin proxy de backend nuevo). Validación de `asset_uid`: "no existe" se rechaza (400 `ASSET_NOT_FOUND`); "Activos no disponible" nunca bloquea la creación del ticket (se guarda sin verificar, consistente con "ninguna caída de una integración debe causar pantalla en blanco"). Migración `009_ticket_asset_uid.sql` (idempotente, guardas `information_schema`, mismo patrón que `006_ticket_target_area.sql`) aplicada dos veces contra el contenedor real de Tickets sin cambios en la segunda corrida. Evidencia: Tickets backend 19/19 y `tsc --noEmit` limpio (backend y frontend); Activos 11/11; MRTI-Obs 12/12; Core 37/37 (sin regresión, no se tocó su backend); cuatro builds correctos; `git diff --check` limpio en los cuatro repos. Contrato real contra los procesos en vivo: autoservicio de Activos sin sesión → 401, con `X-Service-Key` inválida → 401, con `X-Service-Key` válida y `asset_uid` real → 200 con solo campos no sensibles (confirmado sin `win_*`/`ms_*`/`correo_*`/password), con uid inexistente → 404; mismo patrón verificado en `/api/self/devices/:id` de Infra. `mrti-activos-api` y `mrti-obs-api` reiniciados en PM2 (health 200 ambos); contenedores `mrti-tickets-backend-1`/`mrti-tickets-frontend-1` reconstruidos y reiniciados (health 200); Core solo cambió su frontend estático, sin reinicio de proceso. **Pendiente de verificación en vivo** (requiere clic real de un usuario, no se pudo simular por curl sin una sesión de Core real): el flujo completo botón "Crear ticket" → Core preselecciona el activo → `POST /api/tickets-self` guarda `asset_uid` → aparece en la pestaña Tickets del activo. **Fases 2-5 (RH, Monitor/Agent, Legal, Core) quedan pendientes**, se plantean por separado. Rollback: revertir los cuatro commits, reconstruir los cuatro frontends/backends afectados y reiniciar `mrti-activos-api`/`mrti-obs-api` y los contenedores de Tickets; `asset_uid` es una columna aditiva y puede permanecer sin afectar el código anterior. |

| Activos ↔ RH. Vínculo directo activo-empleado sin cuenta de Core (`rh_employee_id`) | Completa | 2026-09-02 | Activos `5552cee`. Pedido puntual del usuario (no forma parte de las Fases 2-5 planteadas para Activos↔RH, que siguen pendientes): vincular activos con la ficha real de un empleado de RH usando `id_empleado`/`usuario_asignado` ya importados de SAP, aunque ese empleado todavía no tenga correo corporativo vinculado a Core. Migración `008_rh_employee_link.sql` (idempotente, guardas `information_schema`, aplicada dos veces sin cambios en la segunda corrida) agrega `rh_employee_id INT UNSIGNED NULL` a `activos`/`activo_asignaciones` — referencia a `employees.id` de RH **por convención, sin FK entre bases**, mismo patrón que `device.asset_id`; reemplaza el CHECK de dos vías (`chk_activos_holder`/`chk_asignaciones_holder`) por uno de tres vías (`_v2`: a lo más uno / exactamente uno de `portal_user_id`, `tercero_id`, `rh_employee_id`). Investigación de solo lectura antes de vincular nada: 143 activos con `id_empleado` sin vínculo, 137 con `employee_number` real en RH sin ambigüedad (0 colisiones de código entre distintos empleados); comparando el nombre por coincidencia de palabras (sin importar acentos/orden) solo 108 confirman — el resto son casos reales de número obsoleto/reutilizado (TI-00009: `id_empleado 0055` pertenece hoy a otra persona en RH) o `usuario_asignado` que no es nombre de persona (`"Dañada"`, nombres de laboratorio, `"SITE"`). Script `server/scripts/reconcile-rh-employees.js` (dry-run por defecto, mismo patrón que `reconcile-obs.js`; lógica de coincidencia pura y testeada en `src/reconciliation.js`, 15/15 pruebas OK) — reporte de dry-run mostrado al usuario antes de aplicar nada; aplicado con `--apply` tras su confirmación explícita ("si por favor aplicalos"). Resultado real: 9 confirmados ya tenían `portal_user_id` en su ficha de RH (vinculados por esa vía existente), 99 sin cuenta de Core (vinculados por `rh_employee_id`, nuevo), 29 con nombre que no coincide + 2 sin candidato quedaron listados para revisión manual, nunca tocados; verificado en la base real tras aplicar (TI-00009 intacto, `rh_employee_id` en 99 filas, 108 filas nuevas en `activo_asignaciones`). `POST /:id/asignaciones`/`DELETE /:id/asignacion` amplían la validación "exactamente uno" a las tres vías. Frontend: `ListPage.jsx` reconoce `rh_employee_id` para el badge "Asignado"; `AssetFormPage.jsx` ya no bloquea "Asignar" a un empleado sin `portal_user_id` — llama al mismo endpoint con `rh_employee_id` y distingue la etiqueta ("Empleado (RH, sin cuenta de Core aún)"). Evidencia: Activos 15/15, build de frontend limpio, `mrti-activos-api` reiniciado en PM2. Rollback: revertir el commit; `rh_employee_id` es aditivo (columna nueva, CHECK reemplazado de forma idempotente) y puede permanecer sin afectar código anterior si se revierte solo la app. |
| Activos ↔ RH. Ficha laboral derivada al asignar | Completa | 2026-09-03 | RH `16c6afb`; Activos `aec2a53`. Se retiraron del formulario de Activos los seis campos manuales duplicados de asignación (empresa, número, nombre, unidad, área y celular). Al elegir o abrir al empleado asignado, Activos consulta en vivo `GET /api/rh-self/asset-assignment-profile` mediante `rh_employee_id` o `portal_user_id`; RH resuelve la relación laboral canónica, empresa y estructura organizacional vigente y conserva valores ausentes como “Sin registrar”. El contrato detallado exige sesión y permiso explícito al módulo Activos; no expone teléfono al directorio general ni crea FK/escrituras entre bases. Las columnas heredadas permanecen para compatibilidad de lecturas/importaciones y rollback, pero ya no son editables en el formulario. Sin migración ni cambio de datos. Evidencia: RH 45/45, Activos 15/15, builds de ambos frontends, sintaxis y `git diff --check` correctos; consulta real de Juan Carlos (RH 830) devolvió empleado 0843, MINEWORKS, unidad MINEWORKS · Chihuahua, área Planta Lixiviacion y teléfono nulo; smoke publicado sin sesión 401, token inválido 401, administrador con Activos 200, health RH/Activos 200 y frontend Activos 200; fixture de Core eliminado (0 residuos). PM2 reiniciado y guardado. Rollback: revertir ambos commits, reconstruir ambos frontends y reiniciar `mrti-rh-api`/`mrti-activos-api`; no hay esquema ni datos que revertir. |
| Activos ↔ RH. Retiro visible de terceros externos | Completa | 2026-09-03 | Activos `8485f57`. La navegación, página de administración y selector “Tercero externo” se retiraron del frontend; toda persona se registra ahora en RH, incluso cuando no procede de CONTPAQi, y el formulario enlaza directamente a `/rh/empleados/nuevo`. La URL compatible `/activos/terceros` redirige al inventario para no caer en la ruta dinámica de un activo. Inventario real previo: 0 terceros, 0 activos y 0 asignaciones vigentes por esa vía. No se borraron tabla, columnas, historial ni endpoints: quedan fuera de la interfaz durante el período compatible. Sin migración ni cambio de datos. Evidencia: Activos 16/16, build y `git diff --check` correctos; bundle publicado sin textos/controles de terceros, con enlace de alta RH; raíz y ruta heredada 200, API antigua sin sesión 401. Rollback: revertir `8485f57` y reconstruir el frontend; backend y datos compatibles permanecen intactos. |
| MRTI Activos. Dashboard operativo y carga de documentos | Completa | 2026-09-03 | Activos `da84875`. “Resumen operativo” se convirtió en **Dashboard de activos** con inventario total/activo/asignado/mantenimiento, cobertura documental, distribuciones por tipo/unidad, actividad reciente y seis alertas principales derivadas de la integración de `_incoming-activos` (sin documentos, datos incompletos, antivirus, Office 365, FortiGate y duplicados); las tres consultas del frontend degradan por separado. La ficha permite cargar PDF/JPG/PNG de hasta 25 MB: el backend valida la firma binaria, genera nombre físico UUID, guarda con permisos `0600` en el almacenamiento privado de Activos, calcula SHA-256, registra usuario y conserva descarga autenticada. La migración aditiva `009_asset_document_uploads.sql` vuelve nullable `sap_id`, agrega `asset_uid` estable, MIME/hash/origen/usuario e índice sin FK cruzada; los 53 documentos importados conservan `sap_id` y se rellenaron con `asset_uid`, mientras los nuevos usan `sap_id NULL` y origen `local`, sin escribir en SAP. Se ejecutó mediante el runner, una segunda vez sin pendientes y directamente una vez adicional sin duplicar columnas/índice; quedaron 53 documentos históricos, 53 vínculos estables y 0 locales antes de uso real. Evidencia: 18/18 pruebas, build, sintaxis, `git diff --check` y `npm audit --omit=dev` con 0 vulnerabilidades. Smoke publicado con administrador desechable: dashboard 200 (270 total, 235 activos, 110 asignados, 125 activos sin asignar y 40 con documentos), archivo falso rechazado 400, PDF válido creado 201 y descargado 200 con SHA-256 idéntico; documento, archivo, auditoría y usuario fixture retirados, quedando los 53 archivos originales. `mrti-activos-api` reiniciado y guardado en PM2; `_incoming-activos` se consultó sólo como referencia y no se modificó. Rollback: revertir `da84875`, reconstruir frontend y reiniciar API; conservar las columnas aditivas y todos los archivos/documentos reales sin borrarlos, pues el código anterior ignora los campos nuevos. |
| MRTI Activos. Tablas ordenables y ocultamiento de claves | Completa | 2026-09-03 | Activos `9007ab9`. Todos los encabezados visibles de los catálogos operativos alternan orden ascendente/descendente igual que Inventario, con flecha y `aria-sort`; el backend ordena antes del límite de resultados y sólo acepta columnas públicas de la lista autorizada. Las tablas de detalle de Alertas ahora incluyen encabezados legibles y ordenan localmente su conjunto ya autorizado. “Ver claves” cambia a “Ocultar claves” y al ocultar elimina el valor revelado del estado del navegador; buscar, ordenar o cambiar de catálogo también limpia todas las claves visibles. Sin migración ni cambio de datos. Evidencia: 19/19 pruebas, build y `git diff --check` correctos; smoke publicado autenticado confirmó 200 y metadatos `marca ASC`/`marca DESC`, además de fallback seguro a `id ASC` ante una columna manipulada; fixture de Core y auditoría eliminados (0 residuos). API reiniciada y guardada en PM2. Rollback: revertir `9007ab9`, reconstruir frontend y reiniciar `mrti-activos-api`; no hay esquema ni datos que revertir. |
| MRTI RH. Historial integral y gestión de plazas | Completa | 2026-09-03 | RH `cad126a`. La nueva vista administrativa `/rh/historial` reúne mutaciones locales, versiones del espejo, altas/bajas/reingresos, cambios de numeración y ejecuciones CONTPAQi con filtros por texto, dominio y fechas; los datos sensibles se redactan y las mutaciones principales conservan antes/después. Puestos separa Todos/Ocupados/Vacantes; el indicador del dashboard abre directamente el filtro y confirmó una vacante real: `PST-000048`, Director de Capital Humano. Un clic en cualquier renglón abre edición. `POST /positions/:id/duplicates` crea 1–20 plazas locales vacantes con clave nueva y jefe opcional; la interfaz muestra cuántas plazas homónimas hay por unidad. La migración aditiva `017_position_local_overrides.sql` se aplicó dos veces: 296 puestos vinculados conservan sincronización hasta que nombre/unidad se ajusten localmente y 54 son locales; una prueba transaccional confirmó que la sincronización no reemplaza el ajuste. CONTPAQi permanece de sólo lectura. RH 45/45, build Vite, sintaxis y `git diff --check` correctos. Smoke publicado: frontend/historial/health 200, historial sin sesión 401 y con administrador 200, filtro de vacantes devolvió el puesto esperado, cantidad inválida 400, duplicación temporal 201 y auditoría con antes/después; plaza, usuario y auditorías fixture eliminados (0 residuos). PM2 online, sin errores nuevos (log de error conserva como último cambio 2026-08-20). Rollback en `MRTI-RH/docs/RH_ACTIVITY_AND_POSITION_WORKFLOW.md`; las columnas aditivas pueden permanecer y las plazas reales creadas no deben borrarse. |
| MRTI Activos. Retiro verificado de `_incoming-activos` | Completa | 2026-09-03 | Activos `adfc1ca`. Se comparó el directorio completo con el módulo oficial y `ActivosTI` en vivo, excluyendo expresamente `.env`, autenticación, usuarios, hojas/campos de cuentas y secretos NVR. La sincronización confirmó 270 activos, 124 componentes, 46 impresoras, 13 Starlink, 7 FortiGate, 3 dominios, 1 mantenimiento, 35 unidades, 53 documentos y 3 configuraciones de alerta; los tres Excel eran instantáneas anteriores y no se conservaron como segunda fuente. Los 53 PDF tienen mapeo único, tamaño y SHA-256 idénticos; el importador ahora verifica ambos extremos, persiste los 53 hashes y falla ante archivos huérfanos o faltantes. Activos 19/19, build Vite, `git diff --check`, sincronización completa y health 200. Tras confirmar 0 faltantes/0 huérfanos, el directorio salió del workspace y quedó recuperable en `/var/www/mrt/.retired/_incoming-activos-20260903` (148 MB); una nueva ejecución sin el origen verificó los 53 archivos oficiales. Rollback: mover el resguardo a `/var/www/mrt/MRTI/_incoming-activos`; los datos oficiales no requieren reversión. |
| Activos ↔ RH. Número real de empleado en inventario | Completa | 2026-09-03 | RH `c141111`; Activos `9e9a41f`. El inventario deja de presentar `id_empleado` heredado como autoridad y resuelve por lote la ficha vigente en RH mediante la referencia estable disponible (`rh_employee_id` o UUID `portal_user_id`). La API protegida `POST /api/rh-self/asset-assignment-profiles` valida y deduplica hasta 1,000 referencias, exige permiso de Activos y devuelve el mismo contrato laboral de solo lectura usado en la ficha individual; si RH no responde, la tabla conserva el valor heredado válido como fallback sin quedar bloqueada. Caso real confirmado: TI-00247 tenía `id_empleado='-'`, estaba enlazado por UUID a Anna Gabriela Portillo Portillo y ahora resuelve el número `2012` de la ficha RH 1357. RH 46/46 y Activos 19/19, builds Vite y `git diff --check` correctos; endpoint real 200 con `2012`, sin sesión 401, fixture Core eliminado (0 residuos), health RH 200 y bundle publicado contiene el contrato nuevo. PM2 RH reiniciado y guardado. Sin migración ni copia de datos entre bases. Rollback: revertir ambos commits, reconstruir Activos y reiniciar RH; la columna heredada continúa intacta. |

| MRTI Activos. Filtro por tipo de alerta | Completa | 2026-09-03 | Activos `ed42d46`. La pantalla de Alertas incorpora filtros “Todas”, Duplicados, Sin documentos, Datos incompletos, Licencias perpetuas, FortiGate, Antivirus y Office 365; cada opción conserva su conteo, las tarjetas también actúan como acceso rápido y “Mostrar todas” restablece la vista completa. La API mantiene el conteo compatible `sin_documentos` y añade `sin_documentos_detalle`, limitado a 500 filas, para que esa categoría muestre activos accionables en lugar de sólo un número. Sin migración ni cambio de datos. Activos 19/19, build Vite y `git diff --check` correctos; smoke autenticado 200 confirmó 195/195 activos sin documento y las otras seis colecciones, sin sesión 401, fixture Core eliminado (0 residuos), health 200, bundle publicado con los controles nuevos y PM2 reiniciado/guardado. Rollback: revertir `ed42d46`, reconstruir el frontend y reiniciar `mrti-activos-api`; el contrato anterior continúa compatible porque el campo numérico no cambió. |
| Transversal. Navegación global en encabezado | Completa | 2026-09-03 | Core `c9f15e4`, Activos `1a105e6`, RH `1977eda`, Monitor `9902057`, Tickets `479bb8c`, Legal `ea64674` y Agent Core `4fde869`. El encabezado `sticky` de cada superficie incorpora un selector nativo que consulta el catálogo autorizado de Core, identifica el módulo actual, mantiene **Mi espacio** y conserva el traspaso seguro de sesión/tema hacia Agent. Las barras laterales quedan reservadas para navegación interna y cuenta, sin repetir todos los módulos; en móvil el selector permanece visible y cede el texto contextual secundario. No hay migración, cambio de API ni datos modificados. Evidencia: Core 37/37, Activos 19/19, RH 46/46, Legal 10/10, Tickets 2/2, typecheck de Monitor y `go test ./...` correctos; siete builds/binarios correctos y `git diff --check` limpio. Smoke publicado: Core, Activos, RH, Monitor, Tickets, Legal, Agent y descargas respondieron 200 con el selector presente; Tickets API 200, sus contenedores quedaron online y `mrti-monitor.service` activo. Rollback: revertir el commit de cada repositorio y reconstruir su frontend; para Agent restaurar `/var/www/mrt/MRTI/bin/mrti-monitor.rollback-module-header-20260903` y reiniciar el servicio. |
...
| 2026-09-01 | Hacer de Core la PWA instalable y mantener las APIs fuera del caché; diferir el push con la aplicación cerrada hasta disponer de HTTPS y eventos servidor→dispositivo | Core ya concentra identidad, navegación y la campanilla, mientras cada módulo conserva la propiedad de sus datos. Guardar tokens en el service worker o sondear APIs protegidas sin una sesión visible ampliaría la superficie de seguridad | El alcance inicial instala el shell, muestra contingencia sin red, usa la cámara mediante una acción explícita y presenta avisos de novedades mientras Core está activo. El push cerrado requerirá suscripciones por dispositivo, VAPID fuera del repositorio y eventos autenticados de Tickets/RH/Legal; no se simula con almacenamiento inseguro de credenciales |
| 2026-09-01 | Permitir temporalmente funciones seguras sobre el HTTP interno sólo mediante políticas corporativas acotadas al origen exacto de MRTI | El administrador pidió aprovechar al máximo la instalación actual sin desplegar HTTPS; activar excepciones globales o pedir banderas manuales distintas en cada navegador sería difícil de auditar y ampliaría el riesgo | Las herramientas versionadas agregan y retiran el origen exacto en las políticas oficiales de Chrome y Edge, requieren elevación, no cierran el navegador y conservan un rollback por valor. Mientras no se apliquen, Core usa únicamente avisos internos. La excepción no sustituye TLS y debe retirarse al publicar HTTPS |
...
| 2026-09-02 | Introducir una llave de servicio compartida (`INTERNAL_SERVICE_KEY`, header `X-Service-Key`) como segundo mecanismo de autenticación entre módulos, en vez de emitir tokens de Core para procesos automáticos | La Fase 1 de la integración de plataforma necesitaba que Tickets resolviera `device_id -> asset_uid` (evento automático de Agent Core, sin usuario navegando) y validara un `asset_uid` recibido sin depender siempre de un token de usuario. Pedirle a Core un token de "usuario de servicio" habría mezclado identidad humana con llamadas de sistema; el patrón `x-agent-key`/`AGENT_API_KEY` ya existente en Tickets resuelve exactamente este problema en sentido inverso | Los endpoints de autoservicio no sensibles nuevos (`GET /api/activos-self/uid/:assetUid`, `GET /api/self/devices/:id`) aceptan sesión real de Core **o** la llave compartida, nunca la relajan para todo el router — cada ruta declara su propio middleware. La llave vive solo en los `.env`/`docker-compose.yml` de Activos, Infra y Tickets, nunca en código ni en Core. Si se filtra, solo permite leer datos no sensibles de esos dos endpoints puntuales — no otorga sesión ni permisos de escritura |
| 2026-09-03 | Mostrar en Activos la ficha laboral de RH en vivo y conservar únicamente referencias estables como vínculo maestro | Empresa, número de empleado, nombre, unidad, área y celular estaban duplicados como entradas manuales del activo; podían contradecir la ficha y obligaban a capturar dos veces la misma información | RH publica un contrato de lectura detallado limitado al permiso de Activos. La interfaz deja de editar esos campos y los deriva al elegir al empleado o abrir una asignación. Las columnas antiguas no se eliminan todavía: siguen como compatibilidad para inventario y fuentes heredadas hasta planear su retiro por expandir-migrar-verificar-retirar |
| 2026-09-03 | Registrar en RH a toda persona asignable, aunque no exista en CONTPAQi, y retirar “Terceros externos” de Activos | RH ya admite altas laborales locales con número temporal `RH-####`; mantener un segundo registro de personas en Activos duplicaba identidad, contacto y flujo operativo | Activos sólo ofrece el selector de RH/Core. El frontend de terceros desaparece y la URL anterior redirige al inventario. La estructura antigua se conserva temporalmente, sin exposición visible ni borrado, para rollback y retiro posterior después de comprobar ausencia de consumidores |
| 2026-09-03 | Hacer a MRTI Activos propietario de sus adjuntos operativos y distinguir documentos importados de cargas locales mediante `document_origin` | La ficha sólo podía consultar archivos heredados y escribirlos de vuelta en SAP habría acoplado el nuevo flujo a una fuente externa; enlazar únicamente por `center_code` también dejaba el documento expuesto a cambios de etiqueta | Los documentos históricos conservan `sap_id` y compatibilidad por código; las cargas locales usan `sap_id NULL`, `asset_uid` estable y almacenamiento privado propio, sin FK ni escritura entre bases. Firma binaria, límite, permisos y SHA-256 protegen el flujo básico; cifrado en reposo, antivirus y respaldo del directorio deben evaluarse antes de tratar estos adjuntos como expediente sensible o ampliar su criticidad |
| 2026-09-03 | Ordenar catálogos en el backend mediante lista permitida y borrar del estado del navegador una clave al ocultarla | Ordenar sólo las primeras 300 filas en el navegador daría resultados parciales, mientras interpolar directamente un título recibido por URL permitiría inyección SQL. Mantener una clave oculta en memoria tampoco corresponde con la acción visible del usuario | Cada catálogo reutiliza exclusivamente sus columnas públicas declaradas y aplica el orden antes de `LIMIT`; los parámetros desconocidos caen al identificador estable. Alertas, que ya llegan acotadas y calculadas, ordenan localmente. Una clave vuelve a solicitarse y auditarse si se desea mostrar otra vez después de ocultarla |
| 2026-09-03 | Consolidar el historial de RH como lectura de registros propietarios ya existentes y modelar los puestos repetidos como plazas independientes con ajustes locales explícitos | Crear una segunda bitácora habría duplicado auditoría, versionado y movimientos CONTPAQi; usar un solo puesto con varios ocupantes simultáneos habría roto la semántica de vacante, jerarquía e historial por plaza. Sobrescribir el nombre local en cada sincronización también volvería inútil la edición solicitada | RH consulta y normaliza sus fuentes inmutables sin mover datos ni exponer valores sensibles. Cada plaza conserva ID y clave propios, un ocupante vigente y su historial; duplicar crea plazas vacantes locales. `name_is_manual` y `org_unit_is_manual` protegen sólo el campo realmente modificado, mientras el resto continúa sincronizado. No existe escritura hacia CONTPAQi |
| 2026-09-03 | Retirar `_incoming-activos` después de comparar datos vivos, documentos y funciones, conservando un resguardo fuera del workspace | La carpeta duplicaba visualmente el módulo, contenía una aplicación obsoleta, dependencias generadas y secretos, pero también había recibido PDF posteriores a la primera integración. Borrarla sin cotejo podía perder documentos; copiarla completa habría perpetuado credenciales y una segunda fuente de verdad | Los datos operativos continúan en el módulo y su espejo sincronizado; cada PDF queda verificado por SHA-256 en el almacén privado. No se trasladan secretos ni snapshots Excel obsoletos. El CRUD directo de catálogos de la aplicación anterior no se reactiva porque esos catálogos son espejos de solo lectura. El resguardo queda fuera del árbol visible para rollback y puede eliminarse definitivamente cuando el propietario decida cerrar también ese periodo de recuperación |
| 2026-09-03 | Resolver en lote desde RH el número visible de empleado de las asignaciones de Activos | `id_empleado` pertenece al inventario heredado y puede contener `-` aunque el activo ya esté correctamente ligado a una ficha por UUID de Core; copiar el número a Activos volvería a duplicar un dato maestro que puede cambiar al conciliar CONTPAQi | RH continúa siendo propietario de `employee_number`. Activos envía únicamente sus referencias estables, indexa la respuesta para la vista actual y presenta el dato vivo; una caída de RH no impide consultar el inventario y sólo activa el fallback heredado. No hay FK ni escritura cruzada |

| 2026-09-03 | Mantener una sola consulta de alertas y filtrar sus categorías en la interfaz | Cada grupo ya se calcula en un mismo contrato y el usuario necesita enfocarse sin perder conteos ni provocar múltiples recargas; separar rutas o páginas duplicaría lógica y haría más lenta la navegación entre tipos | La respuesta añade únicamente el detalle faltante de “Sin documentos” y conserva el resto de campos. La selección “Todas” o por categoría ocurre localmente, con controles accesibles y desplazables en móvil; no modifica ni descarta alertas |
| 2026-09-03 | Ubicar la navegación entre módulos en un selector nativo del encabezado `sticky` y reservar la barra lateral para opciones internas | Una lista completa en cada sidebar crecía con el catálogo, obligaba a recorrerla para cambiar de aplicación y repetía una función global dentro de la navegación local | Todos los módulos consultan el catálogo autorizado de Core y muestran sólo destinos permitidos. El selector acompaña el desplazamiento, funciona con teclado y comportamiento nativo de cierre, y se mantiene compacto en móvil; cada frontend y despliegue continúa independiente |
