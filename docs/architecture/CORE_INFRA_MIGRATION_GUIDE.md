...
| Transversal. CONTPAQi multisitio | Completa | 2026-08-24 | RH `1527edf`. La migración idempotente `007_contpaq_multi_source.sql` se aplicó dos veces; descubre automáticamente bases con la estructura de Nóminas y conserva `MRT` como clave compatible. Carga publicada: 4 sitios, 5,004 relaciones/1,968 personas únicas/311 activas vigentes; 13,461 eventos, 536,054 periodos, 67,166 recibos y 1,894 finiquitos, sin errores. Frontend agrupado/filtrado por sitio, perfiles multirrelación y actualización manual; empleados cada 2 h e historial diario. Pruebas 14/14, build, `git diff --check`, health 200, `/rh/` 200, ruta protegida sin token 401, PM2 online y sin errores nuevos. Rollback: detener RH, retirar del espejo derivado las filas `empresa_code <> 'MRT'`, revertir `1527edf`, reconstruir/reiniciar; CONTPAQi no se modifica y las fuentes pueden rehidratarse. |
