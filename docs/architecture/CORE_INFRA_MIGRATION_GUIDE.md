...
| Transversal. Reportes imprimibles contextuales | Completa | 2026-09-01 | Core `cd7fa2b`, Monitor `e5b56ac`, Activos `9ff8868`, RH `30bb25e`, Legal `251f2da`, Tickets `15a1614` y Agent Core `6835e57`. Las vistas operativas de resumen, listados, auditoría y detalle ofrecen **Imprimir reporte**; altas, edición, configuración, acceso y descargas no muestran una acción sin utilidad. La impresión conserva el contenido y filtros visibles, añade módulo/título/fecha, fuerza una paleta clara y elimina navegación, notificaciones, botones y formularios ajenos al reporte. Builds 7/7; typecheck de Monitor; RH 35/35, Legal 10/10, Tickets 2/2 y Go `go test ./...` correctos. Core, Monitor, Activos, RH, Legal, Tickets y Agent respondieron HTTP 200 y sus bundles publicados contienen el botón y `@media print`; Tickets recreó sólo el frontend y su backend conservó `StartedAt=2026-09-01T16:09:27.566414283Z`; Agent quedó activo con binario idéntico al build. Rollback: revertir los commits indicados, reconstruir cada frontend y restaurar `/var/www/mrt/MRTI/bin/mrti-monitor.rollback-print-reports-20260901` para Agent. |
...
| 2026-09-01 | Imprimir el estado visible de cada vista reportable desde una acción contextual común, sin crear una segunda fuente de datos para documentos | Los módulos ya presentan la información vigente y aplican permisos y filtros propios; generar reportes mediante APIs paralelas duplicaría reglas de alcance y podría producir documentos distintos de lo que el usuario está revisando | Cada módulo invoca la impresión nativa sobre la vista actual y comparte un contrato de papel: encabezado MRTI con módulo, título y fecha; paleta clara; tablas con cabecera repetible; navegación y acciones excluidas. El botón se omite en formularios y pantallas no reportables. Cada repositorio conserva implementación y rollback independientes |
